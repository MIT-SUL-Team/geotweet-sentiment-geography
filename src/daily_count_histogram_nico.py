...
for file in tqdm(files):
    ...
    temp = pd.read_csv(file, compression='gzip', header=0, sep='\t', quotechar='"', lineterminator="\n", usecols=['date', 'longitude', 'latitude', 'message_id'])
    temp['date'] = pd.to_datetime(temp['date']).dt.date

    for i in range(len(temp['longitude'])):
